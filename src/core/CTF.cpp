...
void CTF::setAllCTFPara(float defocus1_now,float defocus2_now,float astig_now,float phase_shift_now,float w_now)
{
    ...
    astig=astig_now/180*M_PI;    // input in degree, store in radius
...
float CTF::computeCTF2D(int x,int y,int Nx,int Ny,bool phaseflip,bool flip_contrast,float z_offset) // z_offset in pixels
{
    int x_norm=x,y_norm=(y>=int(ceil(float(Ny+1)/2)))?(y-Ny):(y);
