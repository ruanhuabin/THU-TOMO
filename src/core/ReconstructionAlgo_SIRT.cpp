...
ReconstructionAlgo_SIRT::~ReconstructionAlgo_SIRT()
{

}

static void volumeProj(
...
{
    float *slice_now[threads];
    for(int th=0;th<threads;th++)
    {
        slice_now[th]=new float[stack_orig.getNx()*stack_orig.getNy()];
    }
    ...
    for(int k=0;k<h_tilt_max;k++)
    {
        ...
        for(int j=0;j<stack_orig.getNy();j++)
        {
            for(int i=0;i<stack_orig.getNx();i++)
            {
                ...
                if(floor(x_final)>=0 && ceil(x_final)<stack_orig.getNx() && floor(y_final)>=0 && ceil(y_final)<stack_orig.getNy() && floor(z_final)>=0 && ceil(z_final)<h)
                {
                    //slice_now[omp_get_thread_num()][j*stack_orig.getNx()+i]=(1-coeff_x)*(1-coeff_y)*(1-coeff_z)*stack_recon[int(floor(z_final))][int(floor(y_final))*stack_orig.getNx()+int(floor(x_final))]+(1-coeff_x)*(1-coeff_y)*(coeff_z)*stack_recon[int(ceil(z_final))][int(floor(y_final))*stack_orig.getNx()+int(floor(x_final))]+(1-coeff_x)*(coeff_y)*(1-coeff_z)*stack_recon[int(floor(z_final))][int(ceil(y_final))*stack_orig.getNx()+int(floor(x_final))]+(1-coeff_x)*(coeff_y)*(coeff_z)*stack_recon[int(ceil(z_final))][int(ceil(y_final))*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*(1-coeff_y)*(1-coeff_z)*stack_recon[int(floor(z_final))][int(floor(y_final))*stack_orig.getNx()+int(ceil(x_final))]+(coeff_x)*(1-coeff_y)*(coeff_z)*stack_recon[int(ceil(z_final))][int(floor(y_final))*stack_orig.getNx()+int(ceil(x_final))]+(coeff_x)*(coeff_y)*(1-coeff_z)*stack_recon[int(floor(z_final))][int(ceil(y_final))*stack_orig.getNx()+int(ceil(x_final))]+(coeff_x)*(coeff_y)*(coeff_z)*stack_recon[int(ceil(z_final))][int(ceil(y_final))*stack_orig.getNx()+int(ceil(x_final))];
                    tmp_y_1_1=
                    (1-coeff_x)
                    *stack_recon[int(floor(z_final))]
                                [int(floor(y_final))*stack_orig.getNx()+int(floor(x_final))]
                    +(coeff_x)
                    *stack_recon[int(floor(z_final))]
                                [int(floor(y_final))*stack_orig.getNx()+int(ceil(x_final))];
        ...
        if(!skip_ctfcorrection)
        {
            ...
            {
                ctf_modulation_omp(slice_now[omp_get_thread_num()],stack_orig.getNx(),stack_orig.getNy(),ctf_para_n,float(k)-float(h_tilt_max)/2.0,plan_fft[omp_get_thread_num()],plan_ifft[omp_get_thread_num()],bufc[omp_get_thread_num()]);
            }
            else
            {
                ctf_modulation_omp(slice_now[omp_get_thread_num()],stack_orig.getNx(),stack_orig.getNy(),ctf_para_n,0.0,plan_fft[omp_get_thread_num()],plan_ifft[omp_get_thread_num()],bufc[omp_get_thread_num()]);
            }
        }
        // projection
        for(int i=0;i<stack_orig.getNx()*stack_orig.getNy();i++)
        {
            proj_omp[omp_get_thread_num()][i]+=slice_now[omp_get_thread_num()][i];
        }
...
void ReconstructionAlgo_SIRT::doReconstruction(map<string, string> &inputPara, map<string, string> &outputPara)
{
    cout<<"Run doReconstruction() in ReconstructionAlgo_SIRT"<<endl;
    ...
    //std::cout <<"h_tilt_max:"<< h_tilt_max << std::endl;
    ...
    it=inputPara.find("num_blocks");
    int num_blocks=1;
    if(it!=inputPara.end())
    {
        num_blocks=atoi(it->second.c_str());
        cout << "Divide into blocks: " << num_blocks << endl;
    }
    else
    {
        cout << "No num_blocks, set default: 1" << endl;
        num_blocks=1;
    }


    // Reconstruction
    cout << endl << "Reconstruction with SIRT:" << endl << endl;
    ...
    if(!unrotated_stack)    // rotated stack
    {
        ...
        if(has_initial_model)
        {
            ...
            for(int j=0;j<stack_orig.getNy();j++)
            {
                ...
                memcpy(stack_recon[j],xz_initial,sizeof(float)*stack_orig.getNx()*h);
            }
            delete [] xz_initial;
            stack_initial.close();
        }
        ...
        for(int it=0;it<it_max;it++)    // round for iteration
        {
            ...
            for(int n=0;n<stack_orig.getNz();n++)
            {
                ...
                // Ax
                cout << "\t\tProjection...";
                float *proj_now=new float[stack_orig.getNx()*stack_orig.getNy()];
                for(int i=0;i<stack_orig.getNx()*stack_orig.getNy();i++)
                {
                    proj_now[i]=0.0;
                }
                float *proj_omp[threads];
                float *bufc[threads];
                fftwf_plan plan_fft[threads],plan_ifft[threads];
                for(int th=0;th<threads;th++)
                {
                    proj_omp[th]=new float[stack_orig.getNx()*stack_orig.getNy()];
                    for(int i=0;i<stack_orig.getNx()*stack_orig.getNy();i++)
                    {
                        proj_omp[th][i]=0.0;
                    }
                ...
                float theta_rad=theta[n]*M_PI/180.0;
                ...
                // loop: h_tilt_max (maximum height for tilted volume)
                //h_tilt_max ~ 4000
                for(int k=0;k<h_tilt_max;k++)   // loop for each xy-slice
                {
                    // rotation
                    // loop: Nx*Ny (whole xy-slice)
                    for(int j=0;j<stack_orig.getNy();j++)
                    {
                        for(int i=0;i<stack_orig.getNx();i++)
                        {
                            float x_now=float(i)-x_offset;
                            float z_now=float(k)-z_offset;  // move origin to center

                            float x_tlt=x_now*cos(-theta_rad)-z_now*sin(-theta_rad);
                            float z_tlt=x_now*sin(-theta_rad)+z_now*cos(-theta_rad);  // tilting

                            float x_final=x_tlt+x_offset;
                            float z_final=z_tlt+z_offset-int(float(h_tilt_max-h)/2.0);
                            ...
                            if(floor(x_final)>=0 && ceil(x_final)<stack_orig.getNx() && floor(z_final)>=0 && ceil(z_final)<h)
                            {
                                //slice_now[omp_get_thread_num()][j*stack_orig.getNx()+i]=(1-coeff_x)*(1-coeff_z)*stack_recon[j][int(floor(z_final))*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*(1-coeff_z)*stack_recon[j][int(floor(z_final))*stack_orig.getNx()+int(ceil(x_final))]+(1-coeff_x)*(coeff_z)*stack_recon[j][int(ceil(z_final))*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*(coeff_z)*stack_recon[j][int(ceil(z_final))*stack_orig.getNx()+int(ceil(x_final))];
                                tmp_z_1=(1-coeff_x)*stack_recon[j][int(floor(z_final))*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*stack_recon[j][int(floor(z_final))*stack_orig.getNx()+int(ceil(x_final))];
                                tmp_z_2=(1-coeff_x)*stack_recon[j][int(ceil(z_final))*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*stack_recon[j][int(ceil(z_final))*stack_orig.getNx()+int(ceil(x_final))];
                                slice_now[omp_get_thread_num()][j*stack_orig.getNx()+i]=(1-coeff_z)*tmp_z_1+(coeff_z)*tmp_z_2;
                            }
                            else
                            {
                    ...
                    if(!skip_ctfcorrection)
                    {
                        if(!skip_3dctf) // perform 3D-ART
                        {
                            ctf_modulation_omp(slice_now[omp_get_thread_num()],stack_orig.getNx(),stack_orig.getNy(),ctf_para[n],float(k)-float(h_tilt_max)/2.0,plan_fft[omp_get_thread_num()],plan_ifft[omp_get_thread_num()],bufc[omp_get_thread_num()]);
                        }
                        else
                        {
                            ctf_modulation_omp(slice_now[omp_get_thread_num()],stack_orig.getNx(),stack_orig.getNy(),ctf_para[n],0.0,plan_fft[omp_get_thread_num()],plan_ifft[omp_get_thread_num()],bufc[omp_get_thread_num()]);
                        }
                    }
                    // projection
                    // loop: Nx*Ny (whole xy-xlice)
                    for(int i=0;i<stack_orig.getNx()*stack_orig.getNy();i++)
                    {
                        proj_omp[omp_get_thread_num()][i]+=slice_now[omp_get_thread_num()][i];
                    }
                ...
                // (A^T)(b-Ax) & x+a*(A^T)(b-Ax)
                cout << "\t\tBack-projection & Refinement...";

                // precompute the height before rotation
                float *stack_height[stack_orig.getNy()]; // (x,z,y)
                for(int j=0;j<stack_orig.getNy();j++)
                {
                    stack_height[j]=new float[stack_orig.getNx()*h];
                }
                #pragma omp parallel for num_threads(threads)
                // loop: Nx*Ny*h (whole volume)
                for(int j=0;j<stack_orig.getNy();j++)
                {
                    for(int k=0;k<h;k++)
                    {
                        for(int i=0;i<stack_orig.getNx();i++)
                        {
                            ...
                            stack_height[j][k*stack_orig.getNx()+i]=z_final;    // [0,h_tilt_max)
                        }
                    }
                }
                // back-projection & ctf-correction & rotation & refinement
                float h_block=float(h_tilt_max)/float(num_blocks);
                // loop: num_blocks (split into several blocks for parallelization)
                for(int t=0;t<num_blocks;t++)    // divide into blocks
                {
                    int h_start=floor(h_block*t);
                    int h_end=ceil(h_block*(t+1));
                    if(h_end>h_tilt_max)
                    {
                        h_end=h_tilt_max;
                    }
                    // back-projection & ctf-correction
                    float *stack_block[h_end-h_start+1];    // (x,y,z)
                    #pragma omp parallel for num_threads(threads)
                    // loop: h_end-h_start+1 (height range for the current block)
                    for(int k=0;k<h_end-h_start+1;k++)
                    {
                        stack_block[k]=new float[stack_orig.getNx()*stack_orig.getNy()];
                        memcpy(stack_block[k],image_now,sizeof(float)*stack_orig.getNx()*stack_orig.getNy());
                        if(!skip_ctfcorrection)
                        {
                            if(!skip_3dctf) // perform 3D-ART
                            {
                                ctf_modulation_omp(stack_block[k],stack_orig.getNx(),stack_orig.getNy(),ctf_para[n],float(k+h_start)-float(h_tilt_max)/2.0,plan_fft[omp_get_thread_num()],plan_ifft[omp_get_thread_num()],bufc[omp_get_thread_num()]);
                            }
                            else
                            {
                                ctf_modulation_omp(stack_block[k],stack_orig.getNx(),stack_orig.getNy(),ctf_para[n],0.0,plan_fft[omp_get_thread_num()],plan_ifft[omp_get_thread_num()],bufc[omp_get_thread_num()]);
                            }
                        }
                    }
                    // rotation & refinement
			...
			//h ~ 1000
                        for(int k=0;k<h;k++)
                        {
                            for(int i=0;i<stack_orig.getNx();i++)
                            {
                                if(stack_height[j][k*stack_orig.getNx()+i]>=h_block*t && stack_height[j][k*stack_orig.getNx()+i]<h_block*(t+1))
                                {
                                    float tmp=0.0;
                                    float z_offset=float(h)/2.0;
                                    float x_now=float(i)-x_offset;
                                    ...
                                    if(floor(x_final)>=0 && ceil(x_final)<stack_orig.getNx() && floor(z_final)>=0 && ceil(z_final)<h_tilt_max)
                                    {
                                        //tmp=(1-coeff_x)*(1-coeff_z)*stack_block[int(floor(z_final))-h_start][j*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*(1-coeff_z)*stack_block[int(floor(z_final))-h_start][j*stack_orig.getNx()+int(ceil(x_final))]+(1-coeff_x)*(coeff_z)*stack_block[int(ceil(z_final))-h_start][j*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*(coeff_z)*stack_block[int(ceil(z_final))-h_start][j*stack_orig.getNx()+int(ceil(x_final))];
                                        tmp_z_1=(1-coeff_x)*stack_block[int(floor(z_final))-h_start][j*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*stack_block[int(floor(z_final))-h_start][j*stack_orig.getNx()+int(ceil(x_final))];
                                        tmp_z_2=(1-coeff_x)*stack_block[int(ceil(z_final))-h_start][j*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*stack_block[int(ceil(z_final))-h_start][j*stack_orig.getNx()+int(ceil(x_final))];
                                    ...
                                    stack_recon[j][k*stack_orig.getNx()+i]+=(lambda*tmp);
                                }
                            }
                        }
                    }
                    for(int k=0;k<h_end-h_start+1;k++)
                    {
                        delete [] stack_block[k];
                    }
                }
                cout << "Done" << endl;
                for(int j=0;j<stack_orig.getNy();j++)
                {
                    delete [] stack_height[j];
                }
        ...
        // write out final reconstruction volume
    ...
    else    // Unrotated stack
    {
        ...
        if(has_initial_model)
        {
            ...
            for(int k=0;k<h;k++)
            {
                ...
                memcpy(stack_recon[k],xy_initial,sizeof(float)*stack_orig.getNx()*stack_orig.getNy());
            }
            delete [] xy_initial;
            stack_initial.close();
        }
        ...
        for(int it=0;it<it_max;it++)    // round for iteration
        {
            ...
            for(int n=0;n<stack_orig.getNz();n++)
            {
                ...
                */
                ...
                // precompute the height before rotation
                float *stack_height[h]; // (x,y,z)
                for(int k=0;k<h;k++)
                {
                    stack_height[k]=new float[stack_orig.getNx()*stack_orig.getNy()];
                }
                #pragma omp parallel for num_threads(threads)
                // Nx*Ny*h (whole volume)
                for(int k=0;k<h;k++)
                {
                    for(int j=0;j<stack_orig.getNy();j++)
                    {
                        for(int i=0;i<stack_orig.getNx();i++)
                        {
                            ...
                            stack_height[k][j*stack_orig.getNx()+i]=z_final;    // [0,h_tilt_max)
                        }
                    }
                }
                // back-projection & ctf-correction & rotation & refinement
                float h_block=float(h_tilt_max)/float(num_blocks);
                // loop: num_blocks (number of blocks for parallelization)
                for(int t=0;t<num_blocks;t++)    // divide into blocks
                {
                    int h_start=floor(h_block*t);
                    int h_end=ceil(h_block*(t+1));
                    ...
                    for(int k=0;k<h;k++)
                    {
                        for(int j=0;j<stack_orig.getNy();j++)
                        {
                            for(int i=0;i<stack_orig.getNx();i++)
                            {
                                if(stack_height[k][j*stack_orig.getNx()+i]>=h_block*t && stack_height[k][j*stack_orig.getNx()+i]<h_block*(t+1))
                                {
                                    float tmp=0.0;
                                    float z_offset=float(h)/2.0;
                                    float x_now=i-x_offset;
                                    ...
                                    {
                                        //tmp=(1-coeff_x)*(1-coeff_y)*(1-coeff_z)*stack_block[int(floor(z_final))-h_start][int(floor(y_final))*stack_orig.getNx()+int(floor(x_final))]+(1-coeff_x)*(1-coeff_y)*(coeff_z)*stack_block[int(ceil(z_final))-h_start][int(floor(y_final))*stack_orig.getNx()+int(floor(x_final))]+(1-coeff_x)*(coeff_y)*(1-coeff_z)*stack_block[int(floor(z_final))-h_start][int(ceil(y_final))*stack_orig.getNx()+int(floor(x_final))]+(1-coeff_x)*(coeff_y)*(coeff_z)*stack_block[int(ceil(z_final))-h_start][int(ceil(y_final))*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*(1-coeff_y)*(1-coeff_z)*stack_block[int(floor(z_final))-h_start][int(floor(y_final))*stack_orig.getNx()+int(ceil(x_final))]+(coeff_x)*(1-coeff_y)*(coeff_z)*stack_block[int(ceil(z_final))-h_start][int(floor(y_final))*stack_orig.getNx()+int(ceil(x_final))]+(coeff_x)*(coeff_y)*(1-coeff_z)*stack_block[int(floor(z_final))-h_start][int(ceil(y_final))*stack_orig.getNx()+int(ceil(x_final))]+(coeff_x)*(coeff_y)*(coeff_z)*stack_block[int(ceil(z_final))-h_start][int(ceil(y_final))*stack_orig.getNx()+int(ceil(x_final))];
                                        tmp_y_1_1=(1-coeff_x)*stack_block[int(floor(z_final))-h_start][int(floor(y_final))*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*stack_block[int(floor(z_final))-h_start][int(floor(y_final))*stack_orig.getNx()+int(ceil(x_final))];
                                        tmp_y_1_2=(1-coeff_x)*stack_block[int(floor(z_final))-h_start][int(ceil(y_final))*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*stack_block[int(floor(z_final))-h_start][int(ceil(y_final))*stack_orig.getNx()+int(ceil(x_final))];
                                        tmp_y_2_1=(1-coeff_x)*stack_block[int(ceil(z_final))-h_start][int(floor(y_final))*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*stack_block[int(ceil(z_final))-h_start][int(floor(y_final))*stack_orig.getNx()+int(ceil(x_final))];
                                        tmp_y_2_2=(1-coeff_x)*stack_block[int(ceil(z_final))-h_start][int(ceil(y_final))*stack_orig.getNx()+int(floor(x_final))]+(coeff_x)*stack_block[int(ceil(z_final))-h_start][int(ceil(y_final))*stack_orig.getNx()+int(ceil(x_final))];
                                        tmp_z_1=(1-coeff_y)*tmp_y_1_1+(coeff_y)*tmp_y_1_2;
                                        tmp_z_2=(1-coeff_y)*tmp_y_2_1+(coeff_y)*tmp_y_2_2;
                                        tmp=(1-coeff_z)*tmp_z_1+(coeff_z)*tmp_z_2;
                                    }
